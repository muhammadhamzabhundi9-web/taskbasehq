...
{id:"legal",exportFormats:["pdf","docx","txt"],name:"AI Legal Draft",icon:"⚖️",desc:"Contracts, NDAs, legal templates",color:"#78716c",label:"What legal document?",ph:"e.g., 'Freelance contract, $5000 project, 6 weeks'",tpl:["Freelance contract","NDA agreement","Terms of service","Privacy policy"],pr:i=>`You are a legal document specialist. Draft for: ${i}\n\nInclude: numbered clauses, definitions, obligations, payment terms, termination, dispute resolution.\n\n⚠️ DISCLAIMER: Template only. Review with qualified attorney.`,type:"text",exportFormats:["pdf","docx","txt"]},
...
{id:"studyguide",exportFormats:["pdf","docx","txt"],name:"AI Study Guide",icon:"📚",desc:"Turn any topic into study materials",color:"#7e22ce",label:"What to study?",ph:"e.g., 'Machine learning basics for beginners'",tpl:["Exam prep","Concept explainer","Practice questions","Study schedule"],pr:i=>`You are a master educator. Study guide for: ${i}\n\n**📋 CORE CONCEPTS:**\n**🧠 MEMORY AIDS:**\n**✅ PRACTICE QUESTIONS (10):**\n**📝 CHEAT SHEET:**\n**📅 7-DAY STUDY PLAN:**`,type:"text",exportFormats:["pdf","docx","txt"]},
...
function dlO(format){
  ...
  if(fmt==='pdf'){
    exportPDF(co,baseName,at.name);
  } else if(fmt==='docx'){
    exportDocx(co,baseName,at.name);
  } else if(fmt==='xlsx'){
    exportXlsx(co,baseName);
  } else if(fmt==='csv'){
    exportCsv(co,baseName);
  } else if(fmt==='html'){
...
function exportPDF(text,filename,title){
...
function exportDocx(text,filename,title){
  if(!window.docx){alert('DOCX library loading... please try again');return;}
  ...
  Packer.toBlob(doc).then(blob=>{
    ...
    const a=document.createElement('a');a.href=u;a.download=filename+'.docx';a.click();
    URL.revokeObjectURL(u);
  });
}

function exportXlsx(text,filename){
