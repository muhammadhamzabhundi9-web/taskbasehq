  ...
  try{
    ...
    if(at.type==='image'){url=IMAGE_API;body={prompt:(at.promptPrefix||'')+i};}
      ...
      if(at.textUpload && window.extractedText){
        // Truncate extracted text to safe size (25K chars to leave room for instruction + system prompt)
        let extractedSafe = window.extractedText;
        if(extractedSafe.length > 25000){
          extractedSafe = extractedSafe.substring(0, 25000) + '\n\n[... document truncated for length ...]';
        }
        promptInput = 'Existing document content:\n\n---\n' + extractedSafe + '\n---\n\nUser instruction: ' + i;
      }
      // Final safety check on full prompt
      const finalPrompt = at.pr(promptInput);
      if(finalPrompt.length > 29500){
        // Truncate to safe size
        body={prompt:finalPrompt.substring(0, 29500)};
      } else {
