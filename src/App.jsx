...
const TOOLS = [
  ...
  { id:3, name:"AI Face Swap", desc:"Swap faces in photos and videos with one click. Ultra-realistic results for creative content and fun projects.", category:"video", icon:"🎭", marketPrice:49, features:["Photo & video swap","Real-time preview","Batch processing","HD quality","Multi-face"], badge:"😱 Mind-Blowing", trending:true, users:"31K+", rating:4.9 },
  { id:4, name:"AI Voice Cloner", desc:"Clone any voice from a 30-sec sample. Generate speech in 50+ languages. So real nobody can tell.", category:"voice", icon:"🗣️", marketPrice:22, features:["Voice cloning","50+ languages","Emotion control","Studio quality","Real-time"], badge:"🎯 Game Changer", trending:true, users:"15K+", rating:4.8 },
  ...
  { id:15, name:"AI Photo Editor", desc:"Edit photos with text commands. Remove objects, change backgrounds, adjust lighting — magic.", category:"image", icon:"🎨", marketPrice:20, features:["Text editing","Object removal","BG swap","Style transfer","Batch"], badge:"🪄 Magic Edit", users:"17K+", rating:4.7 },
  ...
  return(<div>
    <section style={{padding:"44px 16px 36px",textAlign:"center",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",maxWidth:580,margin:"0 auto"}}>
        ...
        <p style={{color:T.muted,fontSize:"clamp(14px,3.5vw,18px)",lineHeight:1.6,margin:"0 auto 6px",maxWidth:460}}>Image generator, video creator, voice cloner, content writer — tools that cost <strong style={{color:T.red,textDecoration:"line-through"}}>${totalMkt}+/mo</strong> separately.</p>
  ...
  return(<div style={{maxWidth:680,margin:"0 auto",padding:"32px 14px",textAlign:"center"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:14,marginBottom:30,textAlign:"left"}}>
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:18,padding:"24px 20px"}}>
        ...
        <button style={{...bs("rgba(255,255,255,0.06)"),border:`1px solid ${T.border}`,width:"100%",padding:13,fontSize:14,marginTop:16}}>Start Free Trial</button>
      </div>
      <div style={{background:`${T.accent}08`,border:`2px solid ${T.accent}30`,borderRadius:18,padding:"24px 20px",position:"relative"}}>
        ...
        <button style={{...bs(`linear-gradient(135deg,${T.accent},${T.cyan})`,`0 8px 25px ${T.accentGlow}`),width:"100%",padding:14,fontSize:15,marginTop:16}}>🚀 Start Free → Then {PLANS[plan].label}{PLANS[plan].period}</button>
...
function Footer({go}){
  return(<footer style={{borderTop:`1px solid ${T.border}`,padding:"28px 14px 20px",maxWidth:960,margin:"0 auto"}}>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))",gap:20,marginBottom:20}}>
      ...
      <div><div style={{color:T.muted,fontSize:10,fontWeight:700,marginBottom:8,letterSpacing:1}}>CONTACT</div><div style={{color:T.dim,fontSize:11,lineHeight:2.2}}>📧 {SITE.email}<br/>💬 {SITE.whatsappDisplay}<br/>📞 {SITE.whatsappDisplay}<br/>🐦 {SITE.twitter}</div></div>
    </div>
    <div style={{textAlign:"center",color:T.dim,fontSize:10,borderTop:`1px solid ${T.border}`,paddingTop:14}}>© 2026 {SITE.name} ({SITE.domain}). All rights reserved. | Privacy | Terms</div>
  </footer>);
}
