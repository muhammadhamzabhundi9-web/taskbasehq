...
const PLANS = {
  free: { price: 0, label: "Free", period: "Free", features: ["All 16+ AI tools", "50 generations/day", "Standard output", "Email support", "50 free credits"] },
  monthly: { price: 9.99, label: "$9.99", period: "/month", features: ["All 16+ AI tools", "Unlimited generations", "Priority processing", "Commercial license", "Priority support", "API access", "New tools monthly"] },
  yearly: { price: 79.99, label: "$79.99", period: "/year", features: ["Everything in Monthly", "Unlimited generations", "Priority processing", "Commercial license", "Priority support", "API access", "Early access to new tools", "Team sharing (up to 3)"] },
};
...
const TOOLS = [
  { id:5, name:"AI Content Writer", desc:"Write blog posts, articles, ad copy — anything. SEO optimized, plagiarism-free, 10x faster.", category:"writing", icon:"📝", marketPrice:49, features:["Blog & articles","Ad copy","SEO optimization","Plagiarism-free","30+ types"], badge:"✍️ Bestseller", trending:true, users:"27K+", rating:4.9 },
  ...
  { id:12, name:"AI Social Manager", desc:"Generate posts, captions, hashtags for all platforms. Schedule content, analyze, grow 10x.", category:"social", icon:"📱", marketPrice:29, features:["Multi-platform","Auto scheduling","AI captions","Analytics","Competitor tracking"], badge:"📱 All-in-One", trending:true, users:"22K+", rating:4.7 },
  ...
  { id:16, name:"AI Grammar & Rewriter", desc:"Fix grammar, rewrite for any tone — professional, casual, academic. Plagiarism check.", category:"writing", icon:"🔤", marketPrice:20, features:["Grammar fix","Tone adjust","Plagiarism check","Readability","Multi-lang"], badge:"✅ Write Better", trending:true, users:"24K+", rating:4.7 },
  { id:19, name:"AI Email Writer", desc:"Professional emails in seconds. Cold outreach, follow-ups, responses — perfect tone.", category:"productivity", icon:"📧", marketPrice:15, features:["Smart templates","Tone control","Follow-ups","Subject lines","Multi-lang"], badge:"📧 Save Hours", trending:true, users:"18K+", rating:4.6 },
  { id:27, name:"AI SEO & Hashtags", desc:"Trending hashtags, SEO optimization, keyword research, competitor analysis. Grow 10x.", category:"social", icon:"#️⃣", marketPrice:12, features:["Trending hashtags","SEO audits","Keywords","Competitors","Rank tracker"], badge:"📈 Rank Higher", trending:true, users:"15K+", rating:4.8 },
  ...
  { id:24, name:"AI PDF Tool", desc:"Chat with PDFs, extract data, summarize, merge, split, fill forms — complete toolkit.", category:"productivity", icon:"📄", marketPrice:15, features:["Chat with PDF","Data extraction","Summarize","Merge/split","Form fill"], badge:"📄 Doc Master", trending:true, users:"16K+", rating:4.7 },
  { id:25, name:"AI Meeting Notes", desc:"Record meetings, get perfect notes, action items, follow-up emails. Never miss details.", category:"productivity", icon:"📋", marketPrice:20, features:["Smart summaries","Action items","Follow-ups","Search","Key points"], badge:"📋 Never Forget", users:"10K+", rating:4.7 },
...
const REVIEWS = [
  { name:"Sarah K.", role:"Freelance Writer, USA", text:"I replaced 5 writing tools with this. Content writer, SEO, emails — all in one. Saving $150+/month.", avatar:"👩‍💻", stars:5, hl:true },
  { name:"Ahmed R.", role:"Marketing Manager", text:"Social media posts, email campaigns, SEO keywords — my whole content strategy runs on TaskBase now.", avatar:"📱", stars:5, hl:true },
  ...
  { name:"Priya S.", role:"Content Strategist", text:"Social media content went from 3 days to 30 minutes. AI captions, hashtags, scheduling — just works.", avatar:"📱", stars:5 },
  { name:"James L.", role:"E-commerce Owner", text:"Product descriptions, ad copy, invoicing — everything for my store in one place.", avatar:"🛒", stars:5 },
  { name:"Fatima H.", role:"HR Manager", text:"Resume screening, meeting notes, email drafts — productivity tripled overnight.", avatar:"📋", stars:5 },
  { name:"David C.", role:"Developer", text:"Code assistant is better than Copilot. Plus writing tools and docs generator? Unreal value.", avatar:"💻", stars:5 },
  { name:"Lisa M.", role:"Real Estate Agent", text:"Writer for descriptions, email follow-ups, presentations for clients. They think I hired an agency.", avatar:"🏠", stars:5 },
];

const FAQ = [
  { q:"What do I get?", a:"Unlimited access to ALL 30+ AI tools. One subscription covers everything. No hidden costs." },
  { q:"How do free credits work?", a:"You get 50 free credits on signup — no credit card needed. Each AI generation uses 1 credit. When credits run out, upgrade for unlimited access." },
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:500,background:"rgba(0,0,0,0.85)",backdropFilter:"blur(10px)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:`linear-gradient(180deg,#111827 0%,${T.bg} 100%)`,borderRadius:"22px 22px 0 0",width:"100%",maxWidth:460,maxHeight:"82vh",overflowY:"auto",padding:"20px 18px 30px",animation:"modalUp 0.3s cubic-bezier(0.16,1,0.3,1)"}}>
        ...
        <button onClick={()=>{onClose();go(PAGES.PRICING);}} style={{...bs(`linear-gradient(135deg,${T.accent},${T.cyan})`,`0 8px 25px ${T.accentGlow}`),width:"100%",padding:15,fontSize:15}}>Start 50 Free Credits →</button>
  ...
  return(<div>
    <section style={{padding:"44px 16px 36px",textAlign:"center",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",maxWidth:580,margin:"0 auto"}}>
        ...
        <p style={{color:T.muted,fontSize:"clamp(14px,3.5vw,18px)",lineHeight:1.6,margin:"0 auto 6px",maxWidth:460}}>Content writer, code assistant, SEO tools, resume builder — tools that cost <strong style={{color:T.red,textDecoration:"line-through"}}>${totalMkt}+/mo</strong> separately.</p>
    ...
    <section style={{padding:"30px 14px",maxWidth:500,margin:"0 auto"}}>
      ...
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:16,padding:"18px 16px"}}>
        {[{n:"Jasper",p:"$49"},{n:"Copy.ai",p:"$36"},{n:"Writesonic",p:"$19"},{n:"Copilot",p:"$19"},{n:"Grammarly",p:"$12"},{n:"Buffer",p:"$15"},{n:"SEMrush",p:"$130"},{n:"Notion AI",p:"$10"}].map((c,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid rgba(255,255,255,0.03)"}}><span style={{color:T.muted,fontSize:13}}>{c.n}</span><span style={{color:T.red,fontSize:13,fontWeight:700,textDecoration:"line-through"}}>{c.p}/mo</span></div>)}
  ...
  return(<div style={{maxWidth:680,margin:"0 auto",padding:"32px 14px",textAlign:"center"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:14,marginBottom:30,textAlign:"left"}}>
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:18,padding:"24px 20px"}}>
        ...
        <button onClick={()=>window.location.href='/login.html'} style={{...bs("rgba(255,255,255,0.06)"),border:`1px solid ${T.border}`,width:"100%",padding:13,fontSize:14,marginTop:16}}>Start Free Trial</button>
      </div>
...
function TermsPage(){
  return(<div style={{maxWidth:700,margin:"0 auto",padding:"32px 14px"}}>
    ...
    <div style={{color:T.muted,fontSize:13,lineHeight:1.8}}>
      ...
      <p>We offer Monthly ($9.99/month) and Yearly ($79.99/year) subscription plans. Both plans include a 50 free credits. You can cancel your subscription at any time from your account dashboard.</p>
      <h3 style={{color:T.text,marginTop:20}}>3. Free Trial</h3>
      <p>New users receive a 50 free credits with full access to all tools. No payment is required during the trial period. If you do not cancel before the trial ends, your selected plan will be charged automatically.</p>
...
function RefundPage(){
  return(<div style={{maxWidth:700,margin:"0 auto",padding:"32px 14px"}}>
    ...
    <div style={{color:T.muted,fontSize:13,lineHeight:1.8}}>
      ...
      <p>All plans include a 50 free credits. During the trial, you have full access to all tools at no cost. If you cancel before the trial ends, you will not be charged anything.</p>
