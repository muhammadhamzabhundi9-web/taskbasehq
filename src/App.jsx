...
function TrackTicket({inp,bs,T}){
  ...
  const check=()=>{
    ...
    try{
      const tickets = JSON.parse(localStorage.getItem('tb_tickets')||'[]');
      const found = tickets.find(t=>t.id===cleanId);
      if(found){
        const days = Math.floor((Date.now()-new Date(found.date).getTime())/(1000*60*60*24));
        setResult({
          id:found.id,
          status:found.status,
          subj:found.subj,
          ...
          pri:found.pri,
          days:days,
          msg:days===0?"✅ Ticket received today. We respond within 2 working days.":days<2?"⏳ Your ticket is being reviewed. We'll respond within 2 working days.":days<=3?"🔄 Your ticket is being processed. Check your email for updates.":"📧 Please check your email — we may have already responded. If not, contact support@taskbasehq.com"
        });
      }else{
        setResult({notFound:true,id:cleanId,msg:"Ticket not found in this browser. Tickets are tracked locally per device. Please check the email confirmation we sent you, or email support@taskbasehq.com with your ticket ID."});
      }
    }catch(e){
      setResult({notFound:true,id:cleanId,msg:"Could not check status. Please email support@taskbasehq.com with your ticket ID for an update."});
    }
  };
  return(<div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:16,padding:"22px 18px"}}>
    ...
    {result&&!result.notFound&&<div style={{marginTop:16,background:"rgba(16,185,129,0.06)",border:"1px solid rgba(16,185,129,0.2)",borderRadius:12,padding:"16px 14px"}}>
