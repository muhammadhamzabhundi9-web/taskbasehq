...
const PLANS = {
  ...
  monthly: { price: 9.99, label: "$9.99", period: "/month", features: ["500 credits/month", "46+ AI tools", "Vision AI Chat", "Image + Voice + Text", "Pro Library", "PDF/Word/Excel export", "Priority support"] },
  yearly: { price: 79.99, label: "$79.99", period: "/year", features: ["500 credits/month", "Save 33% vs monthly", "All Pro features", "Vision AI Chat", "Pro Library", "Priority support"] },
  proplus: { price: 19.99, label: "$19.99", period: "/month", features: ["1,500 credits/month", "3x more than Pro", "All Pro features", "Priority queue", "Dedicated support", "Commercial license"] },
  proplusyearly: { price: 199.99, label: "$199.99", period: "/year", features: ["1,500 credits/month", "Save 17% vs monthly", "All Pro Plus features", "Priority queue", "Dedicated support"] },
  promax: { price: 39.99, label: "$39.99", period: "/month", features: ["5,000 credits/month", "10x more than Pro", "All Pro Plus features", "API access (beta)", "White-label exports", "1-on-1 onboarding"] },
  promaxyearly: { price: 399.99, label: "$399.99", period: "/year", features: ["5,000 credits/month", "Save 17% vs monthly", "All Pro Max features", "API access (beta)", "White-label exports"] },
};
const CATEGORIES = [
...
function SupportPage(){
  ...
  const submitTicket=async()=>{
    if(!tCat||!tName||!tEmail||!tSubj||!tMsg){alert("Please fill all fields");return;}
    const id=genTicketId();
    try{
      // Email to admin
      await fetch("https://api.emailjs.com/api/v1.0/email/send",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({service_id:"service_8zans7l",template_id:"template_a95p4nn",user_id:"c5f49E_C8hbfbmXUE",template_params:{to_email:"muhammadhamzabhundi9@gmail.com",to_name:"Hamza",from_name:tName+" (TaskBase Support)",subject:"🎫 "+id+" | "+tCat.toUpperCase()+" | "+tPri+" | "+tSubj,message:"TICKET: "+id+"
Category: "+tCat+"
Priority: "+tPri+"
Name: "+tName+"
Email: "+tEmail+"
...
Reply to: "+tEmail}})});
      ...
      await fetch("https://api.emailjs.com/api/v1.0/email/send",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({service_id:"service_8zans7l",template_id:"template_a95p4nn",user_id:"c5f49E_C8hbfbmXUE",template_params:{to_email:tEmail,to_name:tName,from_name:"TaskBase HQ Support",subject:"✅ Ticket "+id+" Received",message:"Hi "+tName+",
...
function TrackTicket({inp,bs,T}){
  ...
  const check=()=>{
    if(!tid.trim()){alert("Enter ticket ID");return;}
    // Since we use EmailJS (no DB), show email-based status
    setResult({id:tid.trim().toUpperCase(),status:"submitted",msg:"Your ticket has been received. We respond within 2 working days. Check your email for confirmation."});
  };
  return(<div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:16,padding:"22px 18px"}}>
    ...
    {result&&<div style={{marginTop:16,background:"rgba(16,185,129,0.06)",border:"1px solid rgba(16,185,129,0.2)",borderRadius:12,padding:"16px 14px"}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}><span style={{fontSize:20}}>🎫</span><span style={{color:"#fff",fontWeight:800,fontSize:14}}>{result.id}</span><span style={{background:"rgba(16,185,129,0.15)",color:"#10b981",fontSize:10,fontWeight:800,padding:"2px 8px",borderRadius:20}}>✅ SUBMITTED</span></div>
...
export default function TaskBaseHQ(){
  ...
  return(<div style={{minHeight:"100vh",background:T.bg,fontFamily:T.font,color:T.text,paddingBottom:showInstall?120:0}}>
    ...
    <Footer go={go}/>
    {false&&<InstallBanner onClose={()=>setShowInstall(false)}/>}
  </div>);
}
