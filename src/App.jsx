      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
        {[
          ...
          <div key={i} style={{background:`${p.color}08`,border:`1px solid ${p.best?p.color+"50":p.color+"20"}`,borderRadius:12,padding:"14px 8px",textAlign:"center",position:"relative"}}>
            ...
            <a href={p.link} target="_blank" rel="noopener noreferrer" style={{display:"block",background:`${p.color}15`,border:`1px solid ${p.color}30`,color:p.color,padding:"8px 4px",borderRadius:8,fontSize:11,fontWeight:800,textDecoration:"none"}}>Buy Now</a>
          </div>
        ))}
      </div>
    </div>
...
export default function TaskBaseHQ(){
  const[page,setPage]=useState(PAGES.HOME);const[showInstall,setShowInstall]=useState(false); // PWA disabled
  useEffect(()=>{window.scrollTo({top:0,behavior:"smooth"});},[page]);
  // PWA install banner disabled — mobile app coming soon
